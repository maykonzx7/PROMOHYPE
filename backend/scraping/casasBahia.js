 ...
 */
export default async function scrapeCasasBahia(searchTerms = ['smartphone', 'notebook', 'tv', 'fone', 'tablet']) {
  ...
  try {
    for (const searchTerm of searchTerms) {
      console.log(`Buscando "${searchTerm}" no Casas Bahia...`);
      
      try {
        await page.goto(`https://www.casasbahia.com.br/b/${encodeURIComponent(searchTerm)}/`, {
        ...
        await page.waitForSelector(
          "[data-testid='product-card'], .product-item, [class*='ProductCard'], [class*='product']",
          { timeout: 20000 }
        );
        ...
        await page.evaluate(() => {
          const items = document.querySelectorAll("[data-testid='product-card'], .product-item, [class*='ProductCard']");
        ...
        const products = await page.evaluate(() => {
          ...
          return items.map((item) => {
            try {
              // Título
              const titleEl = item.querySelector("h2, h3, [class*='title'], [data-testid='product-title']");
              ...
              // Buscar preço atual (preço promocional ou normal)
              const priceEl = item.querySelector("[class*='price'], [data-testid='price'], .price, [class*='Price']");
              ...
              const originalPriceSelectors = [
                "[class*='original'], [class*='before'], [class*='de'], [class*='old-price'], [data-testid='original-price']"
              ];
              ...
              const installmentSelectors = [
                "[class*='installment'], [class*='parcela'], [data-testid='installment'], [class*='payment']"
              ];
              ...
              for (const selector of installmentSelectors) {
                ...
                if (installmentEl) {
                  ...
                  // Extrair número de parcelas (ex: "10x de R$ 299,90", "em até 12x")
              ...
              // Link
              const linkEl = item.querySelector("a[href*='/p/'], a[href*='produto']");
        ...
        console.error(`Erro ao buscar "${searchTerm}" no Casas Bahia:`, error.message);
        continue;
      }
    }
    ...
    console.log(`✓ Total de promoções únicas do Casas Bahia: ${uniquePromotions.length}`);
    return uniquePromotions;

  } catch (error) {
    console.error("Erro no scraping do Casas Bahia:", error);
    await browser.close();
    return [];
  }
}
